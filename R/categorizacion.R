...
#'
#' Como los seis índices no están construidos igual, hay tres métodos:
#'
#' - `"valor"`: el índice ya tiene tres valores posibles (0, 50, 100) porque se
#'   construye con dos ítems. Se usa el valor como categoría.
#' - `"conteo"`: ningún ítem de la batería admite el código 85, así que el
#'   denominador es constante y el porcentaje equivale a un conteo. Se corta por
#'   número de situaciones, que es más interpretable.
#' - `"porcentaje"`: todos los ítems admiten 85, así que el denominador cambia
#'   por persona y un conteo no sería comparable. Se corta sobre el porcentaje.
#'
#' @section La aserción:
#' Ningún valor del índice puede caer en más de una categoría. Es el invariante
...
#' @param spec Lista `variable -> list(metodo, cortes, n_items)`, normalmente
#'   `cfg$CORTES`.
...
categorizar_indices <- function(datos, spec) {
    ...
    for (v in names(spec)) {
        ...
        cats <- switch(
            ...
            #* El denominador es constante, así que el % vuelve a conteo sin
            #* pérdida. round() y no floor(): el % viene de una división exacta.
            conteo = cut(
                round(x * s$n_items / 100),
                breaks = c(-1, s$cortes, Inf),
                labels = FALSE
            ),
            porcentaje = cut(
                x,
